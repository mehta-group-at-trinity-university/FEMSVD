...
!            enddo
 ...
 1002 format(a64)
      ...
      subroutine CalcOverlap(Order,xPoints,yPoints,LegPoints,xLeg,wLeg,xDim,yDim,xNumPoints,yNumPoints,u,v,xBounds,yBounds,HalfBandWidth,S)
...
!     double precision TempPot,VInt,VTempInt
      ...
      H = 0.0d0
!     m=-0.5d0/(mu*R*R) is correct
      m=1.0d0
      mr = -0.5d0/(mu*R*R)

            
!      PotFile="points.dat"
!      open(unit=7,file=PotFile(1:index(PotFile,' ')-1))
!!     read(7,*) 
!      do i = 1,161
!         read(7,*) rvec(i),vvec(i)
!      enddo
!      close(unit=7)
!     m = -0.5d0/(mu12*R*R)
...
!                     write(25,*) ix,ixp,H(NewRow,Col)
      ...
      subroutine CalcVMatrix(NumStates,HalfBandWidth,MatrixDim,mPsi,S,Vexp,R,Order,xPoints,yPoints,LegPoints,xLeg,wLeg,xDim,yDim,xNumPoints,yNumPoints,u,v,xBounds,yBounds,Vmid)
